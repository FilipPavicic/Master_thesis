...
class GAN_model(keras.Model):
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            mask_real_label = self.generator([bacgrounds, labels, random_latent_vectors])
            image_real_label = bacgrounds - mask_real_label
            image_real_label = tf.clip_by_value(image_real_label, clip_value_min=0, clip_value_max=1)
            loss = self.loss(real_images, image_real_label)
